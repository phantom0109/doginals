...
async function main() {
    ...
    if (cmd == 'mint') {
        ...
        // Pass sub-command as parameter instead of reading from process.argv
        await wallet(process.argv[3])
    } else if (cmd == 'server') {
        await server()
    } else {
...
function walletBalance() {
    ...
    return({wallet_adress: wallet.address, balance: balance});
}


async function walletSend() {
    // const argAddress = process.argv[4]
    const argAddress = "DLBvVNNJupzk3LdBv2Bnik2r5rKTEw87dc"
    ...
    if (amount) {
        ...
        fund(wallet, tx)
        // console.log("tx with amount", tx) 
    } else {
        tx.from(wallet.utxos)
        tx.change(receiver)
        tx.sign(wallet.privkey)
        // console.log("tx without amount", tx) 
    }    
...
async function broadcast(tx, retry) {
    ...
    headers: {
        ...
        'API-KEY': 'QNs3UmokgDYP2N2Hzb1ofe1apoGtHVtYtdtN7c5u4MroHRwLJMHh',
    },
    };

    // axios.post('https://chain.so/api/v3/broadcast_transaction/DOGETEST', data, config)
    // .then(response => {
...
function server() {
    ...
    router.get('/wallet_sync', async (req, res) => {
        // handle GET request for all users
